...
{ 
 ...
 // float time = 1.0;
  ...
  int NUMDATAPAD =  BLOCKSIZE * NUMBLOCKS;
  
    
  const unsigned long long int bytespad = NUMDATAPAD * (long long int) sizeof(LocationPrim);
  ...
  // allocate memory on the GPU device
  status = cudaMalloc( (void **)(&cordinateLocation), bytespad);
  // checks for cuda errors
  checkCudaErrors( status, "cudaMalloc( (void **)(&cordinateLocation), bytespad)");
 ...
 // int dynSmemSize = 81920;
 // 98304 
  //int dynSmemSize = 99328; 
   //int dynSmemSize = 40448;
   //int dynSmemSize = 49152;
  //int dynSmemSize = 16384;
  //int dynSmemSize = 2048;
  int dynSmemSize = 98304;
  ...
  // call the kernel
  euclideanMatrixDynamicSharedMemory<<< grid, threads, dynSmemSize>>>( cordinateLocation, distanceBtwAllLocation,  NUMDATA, numDataPerThread, NUMDATAPAD);
