...
__global__  void euclideanMatrixDynamicSharedMemory(LocationPrim *cordinates, float* euclideanDistance, long long int NUMDATA, int numDataPerThread) {
   ...
   if (gid < NUMDATA) {
      ...
      for (int i = gid, j = 0; i < NUMDATA; i+=(numDataPerThread*blocksize), j+=1) {

	  
	  for (int n = threadIdx.x, m = i; n < (numDataPerThread*blocksize) && m < NUMDATA; n+=blocksize, m+= blocksize) {
