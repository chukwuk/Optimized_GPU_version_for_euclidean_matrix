...
//  int BLOCKSIZE;
 // int MINGRIDSIZE;
  ...
  //cudaOccupancyMaxPotentialBlockSize( &MINGRIDSIZE, &BLOCKSIZE, euclideanMatrix, 0, 0); 
  ...
  //int dynSmemSize = 65536;
   //int dynSmemSize = 49824;
  ...
  // call the kernel
  euclideanMatrixDynamicSharedMemory<<< grid, threads, dynSmemSize>>>( cordinateLocation, distanceBtwAllLocation,  NUMDATA, numDataPerThread);
  ...
  fprintf(stderr, "New value %.2f   %.2f\n", AllLocationDistance[0][1], HdistanceBtwAllLocation[IDX2C(0,1,NUMDATA)] );      
