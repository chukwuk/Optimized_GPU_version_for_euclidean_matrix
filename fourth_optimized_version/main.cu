...
{ 
  //srand(time(0));
  unsigned long long int NUMDATA = 40000;
 ...
 // cudaFuncSetAttribute(euclideanMatrixSharedMemory, cudaFuncAttributeMaxDynamicSharedMemorySize, dynSmemSize);
 ...
 //int numDataPerThread = dynSmemSize/(sizeof(LocationPrim) * BLOCKSIZE);
  ...
  // call the kernel
  euclideanMatrixStaticSharedMemory<<< grid, threads>>>( cordinateLocation, distanceBtwAllLocation,  NUMDATA, BLOCKSIZE);
